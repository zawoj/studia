% Identyfikacja układu ciągłego G3(s) = 1/(s + 2)
...
% Funkcja do identyfikacji układu G3
function [theta, singular_values] = identify_G3(u, y, ts, num_d_zoh, den_d_zoh, num_d_tustin, den_d_tustin)
...
% Funkcja do walidacji modelu G3
function validate_G3(u, y, theta, ts, t, G3_orig)
    b_id = [theta(1)];
    a_id = [1, theta(2)];
    
    % Model dyskretny
    G3_id = tf(b_id, a_id, ts);
    ...
    title(['Walidacja modelu G3, Ts = ' num2str(ts)], 'FontSize', 14);
    ...
    subplot(4,1,4);
    w = logspace(-1, 2, 100);
    [mag_c, phase_c] = bode(G3_orig, w);
    [mag_d, phase_d] = bode(G3_id, w);
    ...
    filename = ['G3_validation_Ts_' num2str(ts) '.png'];
    saveas(gcf, filename);
    close(gcf);  % Zamknięcie bieżącej figury
    
    % Wyświetlenie MSE jako pojedynczej wartości
    disp(['MSE dla modelu G3 (Ts = ' num2str(ts) '): ' num2str(mse, '%.6f')]);
end
...
for i = 1:length(ts_values)
    ...
    G3_d_zoh = c2d(G3, ts, 'zoh');
    [num_d_zoh, den_d_zoh] = tfdata(G3_d_zoh, 'v');
    
    G3_d_tustin = c2d(G3, ts, 'tustin');
    [num_d_tustin, den_d_tustin] = tfdata(G3_d_tustin, 'v');
    ...
    disp(['===== IDENTYFIKACJA UKŁADU G3 DLA Ts = ' num2str(ts) ' =====']);
    [theta_g3, sv_g3] = identify_G3(u, y, ts, num_d_zoh, den_d_zoh, num_d_tustin, den_d_tustin);
    
    % Walidacja modelu
    validate_G3(u, y, theta_g3, ts, t, G3);
end